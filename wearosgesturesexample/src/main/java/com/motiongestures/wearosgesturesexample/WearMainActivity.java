...
public class WearMainActivity extends WearableActivity implements SensorEventListener {
    ...
    protected void onResume() {
        sensorManager.registerListener(WearMainActivity.this, accelerometer, 10_000);
        sensorManager.registerListener(WearMainActivity.this, gyroscope, 10_000);
        sensorManager.registerListener(WearMainActivity.this, magnetometer, 10_000);
        super.onResume();
    }
                ...
                if(recognitionResponse.getStatus() == Greapi.Status.GestureEnd) {
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for(int i =0;i<size;i++) {
                                mTextView.setText("Recognized gesture " + recognitionResponse.getNames(i) + " with label " + recognitionResponse.getLabels(i));
                            }
                            toggleButton.setChecked(false);
                        }
                    });
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            int size = Math.min(recognitionResponse.getNamesCount(), recognitionResponse.getLabelsCount());
                            for (int i = 0; i < size; i++) {
                                mTextView.setText("Rejected gesture " + recognitionResponse.getNames(i) + " with label " + recognitionResponse.getLabels(i));
                            }
                            toggleButton.setChecked(false);
                        }
                    });
